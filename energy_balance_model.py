...
def standardise(parameters):
    ...
    if len(parameters) == 9:
        ...
        raise ValueError(
            "Number of parameters must be 9 (2-box model) or 11 (3-box model)."
        )
...
def objective(standardised_parameters, y, regularisation_factor):
    ...
    gamma, C, kappa, epsilon, sigma_eta, sigma_xi, F_4xCO2 = unpack_parameters(
        parameters
    )
...
def maximise_likelihood(y, k, regularisation_factor, n_attempts, **kwargs):
    ...
    for attempt in range(n_attempts):
        ...
        for i in range(200):
            ...
            try:
                objective_value = objective(
                    standardised_parameters, y, regularisation_factor
                )
            except ValueError:
                continue
        ...
        try:
            result = minimize(
                objective, initial_guess, args=(y, regularisation_factor), **kwargs
            )
            if result.success:
            ...
            if attempt < n_attempts - 1:
                ...
                print(
                    "  Optimisation failed. Maximum number of attempts reached. Returning most recent result."
                )
                return result
    return result
...
def build_Q_d(A, Q, k):
    ...
    """
    H = np.block([[-A, Q], [np.zeros((k + 1, k + 1)), A.T]])
    G = expm(H)
...
def build_Gamma_0(Ad, Qd, k):
    ...
    Gamma_0 = np.linalg.solve(
        np.eye((k + 1) ** 2) - np.kron(Ad, Ad), Qd.flatten()
    )  # can only use flatten because Qd symmetric
...
def confidence_intervals(parameters, standard_errors, alpha=0.05):
    ...
    return np.array(
        [parameters - z * standard_errors, parameters + z * standard_errors]
    )
